...
def prepare_datasets(dataset_type: DatasetType = DatasetType.TRAINING, save_preprocessing: bool = False):
    ...
    for username in datasets:
        user_directory_path = get_user_datasets_directory(dataset_type, username)
        ...
        list_datasets = get_list_files(user_directory_path)
        if list_datasets:
            total_labels += 1
            for (i, image_name) in enumerate(list_datasets):
                ...
                if save_preprocessing:
                    enhanced_image_path = path.join(preprocessed_images_dir, f"{image_name}_{counter}.4_enhanced.jpg")
                    cv2.imwrite(enhanced_image_path, enhanced_image)

                # Grayscaling
                gray_image = convert_to_grayscale(enhanced_image)
                if save_preprocessing:
                    gray_image_path = path.join(preprocessed_images_dir, f"{image_name}_{counter}.5_gray.jpg")
                    cv2.imwrite(gray_image_path, gray_image)

                # Resize
                resized_image = cv2.resize(gray_image, (settings.HOG_RESIZE_WIDTH, settings.HOG_RESIZE_HEIGHT))
                if save_preprocessing:
                    resized_image_path = path.join(preprocessed_images_dir, f"{image_name}_{counter}.6_resized.jpg")
                    cv2.imwrite(resized_image_path, resized_image)

                # HOG Features
                (hog_desc, hog_image) = get_hog_features(resized_image)
                if save_preprocessing:
                    hog_path = path.join(preprocessed_images_dir, f"{image_name}_{counter}.7_hog.jpg")
