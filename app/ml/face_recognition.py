...
def recognize(image, semester_code: str, course_code: str):
    ...
    user_image = image
    capture_path = path.join(settings.ML_TEST_OUTPUT_FOLDER, f"{current_datetime}_1.face.jpg")
    ...
    adjusted_path = path.join(settings.ML_TEST_OUTPUT_FOLDER, f"{current_datetime}_2.adjusted.jpg")
    cv2.imwrite(adjusted_path, user_image)
    user_image = cv2.cvtColor(user_image, cv2.COLOR_RGB2GRAY)
    gray_path = path.join(settings.ML_TEST_OUTPUT_FOLDER, f"{current_datetime}_3.gray.jpg")
    cv2.imwrite(gray_path, user_image)
    resized_image = cv2.resize(user_image, (settings.HOG_RESIZE_WIDTH, settings.HOG_RESIZE_HEIGHT))
    resized_path = path.join(settings.ML_TEST_OUTPUT_FOLDER, f"{current_datetime}_4.resized.jpg")
    ...
    (hog_desc, hog_image) = feature.hog(resized_image, orientations=settings.HOG_ORIENTATIONS, pixels_per_cell=settings.HOG_PIXELS_PER_CELL,
                                        ...
                                        visualize=True)
    hog_path = path.join(settings.ML_TEST_OUTPUT_FOLDER, f"{current_datetime}_5.hog.jpg")
